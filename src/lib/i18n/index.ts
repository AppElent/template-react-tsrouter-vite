...
export const useLanguageChange = () => {
  const changeConfigLanguage = useConfigStore((state) => state.changeLanguage);
  useEffect(() => {
    if (changeConfigLanguage) {
      changeConfigLanguage(t); // Pass the updated t function
    }
  }, [i18n.language, t, changeConfigLanguage]); // Trigger on language or t change
};
